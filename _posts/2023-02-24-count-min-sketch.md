...
This is where Count-min sketch comes into the picture. The count-min sketch is a probabilistic data structure that can **estimate** the frequency of elements by using sublinear space at the expense of over-counting some elements due to hash collisions.
...
2. An array of seed values of type `uint64` that will be used to generate the hash of the data element.
...
The idea behind `increment` operation can be summarized as:
...
The idea behind `estimate` operation is very similar to `increment` operation.
...
func TestGetsTheEstimateForKeysInAStream(t *testing.T) {
    ...
    for _, key := range stream {
	    ...
	    if count < expectedCounts[key.AsString()] {
                t.Fatalf(
                    "Expected atleast the count %v for key %v, received %v", 
...
4. Assert the estimate of count for all the keys
  - As a part of the assertion we want to ensure that the estimated count is "at least" equal to the expected count. In case of hash collisions between two keys, estimate of the count might be higher than the expected count. Hence, we expect that the estimated count is "atleast" equal to the expected count.
...
- Let's use the *lower four bits to store counter for even positions of the matrix* and *higher four bits for odd positions of the matrix*.
...
- If the number is odd, we need to increment the higher four bits only if the counter represented by those bits has not reached 15. In order to ensure that the counter represented by the higher four bits has not reached 15, we can perform a right shift by four, followed by an AND operation with 0x0f.
...
func newCountMinSketch(counters int) *CountMinSketch {
	...
	//create a new byte array using make function and set the newly created byte array at the current index of the matrix
...
>Every row in the matrix has a byte array of size `updatedCounters/2`. If we are using the four bit counter approach, then each byte stores the counters for 2 keys. This means that we can reduce the total number of counters by 2. Assume input counter as 18. This results in the value of `updatedCounters` to be 32. With four bit counter approach we end up with 16 cells for each row with each cell containing a byte.
...
2. Maintains high cache-hit ratio
...
One of the interesting requirements was "maintaining a high cache-hit ratio". In order to achieve this goal, the development team implemented an LFU (least frequently used) based eviction policy called [TinyLFU](https://blog.dgraph.io/refs/TinyLFU%20-%20A%20Highly%20Efficient%20Cache%20Admission%20Policy.pdf).
...
Any access to a key in LFU based caching solution should increment the frequency of the accessed key. Instead of implementing the accurate access count for a key, Ristretto decided on using estimated access count.  

In order to increase the access count of a key, Ristretto batches the `gets` in a buffer. As soon as the buffer is full, the buffered keys are handed off to a goroutine for incrementing their frequencies. This is why the `Push` method of `tinyLFU` receives a slice of keys of type `uint64` (*hash of the incoming keys*) . 

`Push` method results in incrementing the access frequency of the keys by invoking the `Increment` method on the `cmSketch` object. `cmSketch` (`sketch.go`) is an implementation of count-min sketch.

>With Ristretto, keys are already hashed so the type of the parameter for key in all these methods is `uint64`.

`tinyLFU` also provides a method called `Estimate` that is responsible for estimating the frequency of a given key. In order to get the estimate, `tinyLFU` invokes the `Estimate` method on the `cmSketch` object and returns the estimated frequency of the key.

### Code

Code for this article is available [here](https://github.com/SarthakMakhija/probabilistic-data-structures/tree/main/frequency).
