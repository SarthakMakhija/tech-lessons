...
Welcome all to this article named <i>AWS Lambda - A Dialogue</i> and let me introduce our guests Mr. Hernandez and Ms. Jessica who would walk us through their experiences of using AWS lambda. 

Welcome Hernandez and Jessica and thank you for participating in this <i>Virtual Podcast</i>. Let's begin folks.
...
Jessica> That's nearly right. When you create a lambda function, you need to specify a handler which is nothing but the <b>filename.exported function name</b> which acts as an entry point for your application. 

Let's say, you have a file named "handler.js" and it exports a function named "processOrders", your handler becomes <b>handler.processOrders</b>.
...
Me> Hernandez, since a lambda function is not always running, does it incur any kind of latency to serving a request?
...
Let's say, we set concurrency limit of 100 for a lambda function, all we are saying is the lambda function will have a total of 100 pizza slices which means you can have 100 concurrent executions of lambda function. Concurrency limit set for a lambda function is reduced from concurrency pool, which is 1000 for all lambda functions per AWS account.            
...
If it is a synchronous request, client receives a timeout error (code 429) and in case of asynchronous request, Lambda (the compute service) retries yours lambda function, I think twice, before sending the request event or message to a DLQ, assuming DLQ is configured.

<blockquote class="wp-block-quote">
    <p>Various configuration options can be specified while creating a lambda functions including IAM role, memory, timeout, VPC concurrency etc.</p>
</blockquote>
...
Me> Nice. How do set up AWS X-Ray with lambda function? Do you need to set up an X-Ray agent or something like that?

Hernandez> No, with lambda function, you need to do a very few things in order to set up tracing -

+ <b>IAM role</b> in order to send traces to AWS X-Ray
...
Me> Jessica, any restrictions that you have faced with AWS lambda?
...
Jessica> True. I think these <b>constraints are very sensible</b> and if we are hitting some of them, it is worth looking back and seeing if there is another problem somewhere else. 
...
Jessica> We have actually built <b>serverless microservices</b> using AWS Lambda and for processing <b>web clicks</b> on our application. 
